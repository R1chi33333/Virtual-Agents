...
In immersive VR specifically, agents serve as social partners or guides in highly realistic settings. The strong sense of presence induced by VR means that users react very authentically to virtual humans and environments[^17], so the design of an agent's form can have a pronounced impact on user experience. Moore and Urakami found that in a multitasking cognitive setting a voice-only assistant caused significantly less user distraction than an embodied agent[^38], whereas Robb et al. (2023) showed that in a collaborative task a humanoid agent with facial cues was trusted more and boosted engagement compared to a voice-only smart speaker. These results underscore that no single “pet-like” or anthropomorphic agent design fits all applications – agent form and modality must match the context.
