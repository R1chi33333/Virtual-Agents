...
## 2. Literature Review
### 2.1 Virtual Agents 
Virtual agents are broadly defined as computer-generated characters that engage users in lifelike interactions within a digital environment[^7]. These agents, ranging from simple chatbots to richly animated 3D agents, typically assume social roles (e.g., guide, companion, or adversary) that shape the user’s experience or narrative[^8]. Virtual agents can be either embodied, capable of nonverbal cues (gesture, gaze, posture) to enrich communication, or disembodied, such as voice assistants. Prior work has shown that simply giving an agent a humanlike form causes users to treat it more like "another person," and that adding natural behaviors (e.g., expressive gestures or emotion) increases users' perceptions of the agent's realism and likability[^9]. Likewise, agents combining verbal and visual channels tend to elicit a strong sense of social presence, and therefore users often subconsciously apply social norms to them as if they were real humans[^10][^11]. Disembodied voice assistants such as Siri and Alexa, which already have hundreds of millions of users worldwide[^13], demonstrate the widespread adoption and practical relevance of virtual agents in daily life. In practice, virtual agents have been deployed to support learning, therapy, and collaboration across diverse platforms. In immersive VR settings, studies report that embodied agents can reduce user cognitive load[^14] and improve engagement and task performance by boosting immersion and rapport[^15].


In immersive VR specifically, agents serve as social partners or guides in highly realistic settings. The strong sense of presence induced by VR means that users react very authentically to virtual humans and environments[^17], so the design of an agent's form can have a pronounced impact on user experience. Moore and Urakami found that in a multitasking cognitive setting a voice-only assistant caused significantly less user distraction than an embodied agent[^38], whereas Robb et al. showed that in a collaborative task a humanoid agent with facial cues was trusted more and boosted engagement compared to a voice-only smart speaker[^3].

To cover the relevant design space, we select four representative agent embodiments. First, a fully embodied humanlike agent allows use of gaze and gesture, for example, the agent 'Jake' introduced by Wang et al. was given head turns and hand waves to enhance perceived helpfulness and trust[^10]. Second, an animal-companion agent is included, virtual animals (pets) are known to evoke empathy and social support, and studies even propose VR dogs as walking companions to reduce loneliness[^18]. Third, an object-based agent (for example, a talking 'smart speaker' agent) embodies an agent in a familiar object, prior AR work modeled a cylindrical smart-home device (the agent 'Zee' introduced by Wang et al.) representing an Alexa-like agent[^10]. Finally, a voice-only agent (no visual form) represents the common disembodied assistants (the agent 'Ava' introduced by Wang et al., similar to Siri)[^10]. These four forms match those used in related studies – e.g. Ruiz et al. explicitly compared a voice-only assistant, a smart speaker cylinder, a full-size agent and a miniature agent in an AR task and each has been shown to produce distinct user responses (different gaze patterns, trust levels) in prior work[^10]. By testing these four representation types together, we ensure our experiments span the key variations of embodiment and anthropomorphism that HCI theory suggests will affect social and experiential outcomes in VR.

### 2.2 Identity Continuity

In HCI, an agent’s identity cues (name, voice, color scheme) shape user perceptions[^19]. In narrative theory and psychology, maintaining a consistent character across scenes strengthens the user’s sense of continuity[^20]. For example, in storytelling, a character’s persistent identity often yields greater emotional investment[^21].

Bickmore and Picard found that users interacting with a relational virtual agent (one designed with a consistent social-emotional identity) reported significantly higher trust, likability, and desire to continue interacting than with a task-oriented agent[^32]. However, frequent identity switching may undermine the cumulative rapport of a relationship, since sustained trust in relational-agent frameworks depends on consistent social cues[^32].

Conversely, allowing agents to switch personas can enhance perceived expertise and contextual fit: Liew and Tan demonstrated that specialist agents (with domain-specific identities) are judged as more expert and credible than a single generalist agent[^40]. Braun et al. showed that when a virtual assistant’s fixed personality does not match the user’s context or needs, likability and trust drop significantly, implying a loss of novelty or role-appropriateness[^39].

Existing immersive virtual environments research has examined agent embodiment (e.g. how animal vs. human agents influence empathy) and social presence, but typically with a single agent form[^33]. Studies on robot consistency show that repeated error-free interactions with the same persona can build trust[^23], while changing persona might confuse users[^35].

Little prior work has examined identity consistency across multi-form agents in VR. We draw on analogous findings from social robot research (e.g. user trust in physically embodied agents) to posit that identity continuity in VR will similarly affect relational outcomes.
...
Prior HCI studies have explored how different agent embodiments and behaviors affect user experience[^3] [^12] [^9] [^33]. For example, research on companion agents suggests that social presence and realism enhance engagement[^33]. However, most VR agent studies use a single agent type. Studies on nonverbal communication in VR show that consistent cues (like colour scheme, markings on the body, or particular features such as a specific style of eyes) help users follow an agent’s identity[^34]. In related work, an “artificial social agent questionnaire” (ASAQ) was developed to measure users’ sense of social bond with an agent[^28]. These ASAQ constructs (e.g. willingness to interact, perceived social presence) inform our connectedness measure. Prior work has typically investigated either the effects of changing an agent’s appearance, or the impact of identity continuity, but seldom the interplay between changing appearances and maintaining a continuous identity. We aim to fill this gap by comparing identity-continuous vs discontinuous conditions, leveraging a mixed-agent VR setting. This will extend theory on identity design: if our results support continuity, it implies that users form a single “virtual persona” that spans forms, enriching theories of multi-agent interaction in VR.
...
- **C2: Identity-Continuous Condition: Participants interact with virtual agents maintaining consistent identity cues across all four VR scenarios.**
...
In the C2 (identity‐continuous) condition, the same agent reappears in different guises (human, animal, object, voice) but always carries the same identity cues. In the C1 (identity‐discontinuous) condition, the agent’s appearance and key features change each scene to signal a new persona, and each new persona explicitly introduces itself via a brief spoken greeting. The four scenes are presented in a randomized order for each participant to counterbalance any order effects.
...
- **Qualitative Analysis**: Dialogue transcripts will be coded for credibility-related speech acts. Each user utterance will be labeled as a Trust cue or Doubt cue. We will analyze whether users’ trust differs between agent conditions based on these spoken cues.
...
Beyond these primary hypotheses, the study will also provide insights into the magnitude of these effects, helping us understand not only whether identity continuity matters, but how large its impact is in practice. We will be able to explore which aspects of the agent (e.g., voice, name, behavioral style) are most influential in supporting the sense of ongoing identity, and gather subjective feedback on what participants like or dislike when interacting with continuous vs. discontinuous agents. In addition, by analyzing patterns over multiple interactions, we may gain initial insights into how users form relationships with agents over time, and what might disrupt or facilitate that process, even if the expected main effects are not observed.

Theoretically, these insights will extend understanding of agent design by showing whether identity design, not just visual fidelity or behavior influences social metrics. This complements existing HCI theories on trust and anthropomorphism, adding empirical evidence about persona continuity. We effectively test whether a single persona transcending form enhances user–agent rapport.
...
Finally, this study approach contributes to research methodology in VR as well. Overall, we expect to contribute both empirically and practically to the field of HCI and VR agent design.
...
[^38]: Billie Akwa Moore, Jacqueline Urakami (2022) , The impact of the physical and social embodiment of voice user interfaces on user distraction, International Journal of Human-Computer Studies, Volume 161, 102784, ISSN 1071-5819, doi: 10.1016/j.ijhcs.2022.102784.

[^39]: Michael Braun, Anja Mainz, Ronee Chadowitz, Bastian Pfleging, and Florian Alt. 2019. At Your Service: Designing Voice Assistant Personalities to Improve Automotive User Interfaces. In Proceedings of the 2019 CHI Conference on Human Factors in Computing Systems (CHI '19). Association for Computing Machinery, New York, NY, USA, Paper 40, 1–11. doi: 10.1145/3290605.3300270

[^40]: Tze Wei Liew, Su-Mae Tan, Exploring the effects of specialist versus generalist embodied virtual agents in a multi-product category online store, Telematics and Informatics, Volume 35, Issue 1, 2018, Pages 122-135, ISSN 0736-5853, doi: 10.1016/j.tele.2017.10.005.
