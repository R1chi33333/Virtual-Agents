...
In HCI, an agent’s identity cues (name, voice, color scheme) shape user perceptions[^19]. In narrative theory and psychology, maintaining a consistent character across scenes strengthens the user’s sense of continuity[^20]. For example, in storytelling, a character’s persistent identity often yields greater emotional investment[^21]. Bickmore and Picard found that users interacting with a relational virtual agent (one designed with a consistent social-emotional identity) reported significantly higher trust, likability, and desire to continue interacting than with a task-oriented agent[^32]. This continuity builds familiarity and trust, but it can also constrain flexibility. Braun et al. showed that when a virtual assistant’s fixed personality does not match the user’s context or needs, likability and trust drop significantly, implying a loss of novelty or role-appropriateness[^39]. Conversely, allowing agents to switch personas can enhance perceived expertise and contextual fit: Liew and Tan (2018) demonstrated that specialist agents (with domain-specific identities) are judged as more expert and credible than a single generalist agent[^40]. However, frequent identity switching may undermine the cumulative rapport of a relationship, since sustained trust in relational-agent frameworks depends on consistent social cues[^32].
Little prior work has examined identity consistency across multi-form agents in VR. Existing immersive virtual environments research has examined agent embodiment (e.g. how animal vs. human agents influence empathy) and social presence, but typically with a single agent form[^33]. Studies on robot consistency show that repeated error-free interactions with the same persona can build trust [^23], while changing persona might confuse users [^35]. We draw on analogous findings from social robot research (e.g. user trust in physically embodied agents) to posit that identity continuity in VR will similarly affect relational outcomes.
